    #[tokio::test]
    async fn tavily_http_research_result_returns_error_when_pinned_key_unavailable_without_fallback() {
        let db_path = temp_db_path("http-research-result-pinned-key-unavailable");
        let db_str = db_path.to_string_lossy().to_string();

        let pinned_secret = "tvly-http-research-result-disabled-key";
        let fallback_secret = "tvly-http-research-result-active-fallback";
        ...
        let access_token = proxy
            .create_access_token(Some("http-research-result-pinned-unavailable"))
            .await
            .expect("create token");
        ...
        let request_id = "req-pinned-unavailable";
        ...
        sqlx::query("UPDATE api_keys SET status = 'disabled', status_changed_at = ? WHERE id = ?")
            .bind(now)
            .bind(&pinned_key_id)
            .execute(&pool)
            .await
            .expect("disable pinned key");
        ...
        assert_eq!(
            ...
            "research result must not fall back to another key when the pinned key is unavailable",
        );

        let request_log_count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM request_logs WHERE path = ?")
                .bind(format!("/api/tavily/research/{request_id}"))
                .fetch_one(&pool)
                .await
                .expect("count request logs");
        assert_eq!(
            request_log_count, 0,
            "no upstream request should be logged when pinned-key acquisition fails",
        );
