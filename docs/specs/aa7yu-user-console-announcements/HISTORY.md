...
- 公告正文编辑器同时保留 Markdown、左右对比、所见即所得三种模式；列表页公告预览直接复用用户控制台弹窗/横幅公告组件，避免维护另一套用户侧预览实现。
- Storybook 静态构建使用轻量 Markdown 编辑器替身，真实 Milkdown 行为通过本地 demo mock 页面验证并沉淀视觉证据。
- 横幅公告条幅只保留标题作为低打扰入口；有正文时通过详情入口打开弹窗，避免长正文挤占控制台顶部空间；用户确认或关闭详情弹窗后视为关闭该横幅公告，避免同一公告在用户看完详情后继续打扰；无正文时保留直接关闭能力，避免打开空详情。
