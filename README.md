...
- **Segmented reconciliation for anonymized billing** – when `accessToken` mode and 100% API/MCP rebalance are fully active, Hikari records real `(token, key, period)` usage, settles each full business period once, and applies signed quota adjustments back to the original window.
...
- **Admin privacy status** – `/admin/system-settings/privacy-status` shows the configured/effective upstream identity policy, rollout gates, settlement queue state, and recent reconciliation adjustments without exposing raw upstream credentials.
